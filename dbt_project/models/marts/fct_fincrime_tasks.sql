with matches as (
    select task_id, count(*) as candidate_count,
           case when count(*) = 1 then min(workflow_execution_id) end as inferred_workflow_execution_id
    from {{ ref('int_task_workflow_candidates') }} group by task_id
)
select t.task_id, t.task_type, t.task_state, t.resolution, t.created_at, t.updated_at,
    cast(t.created_at as date) as task_date,
    t.user_id, t.transaction_id,
    coalesce(m.candidate_count, 0) as candidate_count,
    case when t.has_context_conflict then 'CONFLICTING_CONTEXT'
