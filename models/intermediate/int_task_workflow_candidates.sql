...
join {{ ref('int_workflow_context') }} w on t.transaction_id = w.transaction_id
where t.transaction_id is not null and not t.has_context_conflict
...
select t.task_id, w.workflow_execution_id
from {{ ref('int_fincrime_task_context') }} t
join {{ ref('int_workflow_context') }} w on t.user_id = w.user_id
