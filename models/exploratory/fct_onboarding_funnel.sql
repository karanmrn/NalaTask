with starts as (
    select funnel_entity_id, min(case when event_type = 'sign_up.started' then event_time end) as signup_started_at,
           min(resolved_user_id) as user_id
    from {{ ref('int_onboarding_events') }} group by funnel_entity_id
), signups_ranked as (
    select e.funnel_entity_id, e.event_time, e.country,
        row_number() over (partition by e.funnel_entity_id order by e.event_time, e.event_id) as rn
    from {{ ref('int_onboarding_events') }} e
    join starts s on e.funnel_entity_id = s.funnel_entity_id
    where e.event_type = 'sign_up.completed'
      and (s.signup_started_at is null or e.event_time >= s.signup_started_at)
), signups as (
    select s.funnel_entity_id, s.user_id, s.signup_started_at,
           r.event_time as signup_completed_at, r.country as signup_country
    from starts s left join signups_ranked r on s.funnel_entity_id = r.funnel_entity_id and r.rn = 1
    where s.signup_started_at is not null or r.event_time is not null
), kyc as (
    select s.funnel_entity_id, min(e.event_time) as kyc_completed_at
    from signups s left join {{ ref('int_onboarding_events') }} e
      on s.funnel_entity_id = e.funnel_entity_id and e.is_final_kyc_event
      and e.event_time >= s.signup_completed_at
    group by s.funnel_entity_id
), tx as (
    select s.funnel_entity_id,
    ...
    from signups s join kyc k on s.funnel_entity_id = k.funnel_entity_id
    left join {{ ref('int_onboarding_events') }} e on s.funnel_entity_id = e.funnel_entity_id
      and e.event_type = 'transaction.completed'
    group by s.funnel_entity_id
)
select s.funnel_entity_id, s.user_id, s.signup_country,
    s.signup_started_at, s.signup_completed_at, k.kyc_completed_at,
    t.first_transaction_completed_at, t.funnel_transaction_completed_at,
...
from signups s join kyc k on s.funnel_entity_id = k.funnel_entity_id
join tx t on s.funnel_entity_id = t.funnel_entity_id
