...
measured as (
    select
        *,
        ...
        case
            when attempt_state = 'COMPLETED' and updated_at >= created_at
                then {{ dbt.datediff('created_at', 'updated_at', 'millisecond') }} / 1000.0
        end as completion_seconds,
        case
            when attempt_state = 'FAILED' and updated_at >= created_at
                then {{ dbt.datediff('created_at', 'updated_at', 'millisecond') }} / 1000.0
        end as failure_seconds
    from attempts
)
